...
def main() -> None:
    ...
    if branch == 'main':
        if prerelease:
            print(f'❌ Main branch cannot have a pre-release version: {toml_parser.library_version}')
            sys.exit(1)
        else:
            print(f'✅ Main branch, release version: {toml_parser.library_version}')
            build_enabled = 'true'
    else:
        if prerelease:
            print(f'✅ Non-main branch with pre-release version: {toml_parser.library_version}')
            build_enabled = 'true'
        else:
            print(f'⚠️ Non-main branch with release version: {toml_parser.library_version} – build skipped')
            build_enabled = 'false'
    ...
    if github_output:
        ...
        print(f'library-version={toml_parser.library_version}')
        print(f'python-versions={toml_parser.python_versions}')
        print(f'build-enabled={build_enabled}')
